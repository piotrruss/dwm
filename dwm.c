	...
	if (ev->window == root) {
		...
		if (updategeom() || dirty) {
			...
			for (m = mons; m; m = m->next) {
				...
				XMoveResizeWindow(dpy, m->barwin, m->wx, m->by + vp, m->ww, bh);
			}
			focus(NULL);
			arrange(NULL);
		}
...
togglebar(const Arg *arg)
{
	...
	updatebarpos(selmon);
	XMoveResizeWindow(dpy, selmon->barwin, selmon->wx, selmon->by + vp, selmon->ww, bh);
	arrange(selmon);
}
...
updatebars(void)
{
	...
	for (m = mons; m; m = m->next) {
		if (m->barwin)
			continue;
		m->barwin = XCreateWindow(dpy, root, m->wx, m->by + vp, m->ww, bh, 0, DefaultDepth(dpy, screen),
